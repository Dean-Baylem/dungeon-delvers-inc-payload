...
export default function CharacterPublicSection({
  publicContents,
  characterId,
  charPath,
}: {
  ...
  const [content, setContent] = useState(publicContents);
  const [editedContent, setEditedContent] = useState(publicContents);
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('/api/characters/update-content', {
        method: 'PATCH',
        body: JSON.stringify({ content: editedContent, characterId, charPath, type: 'public' }),
      });
  ...
  return (
    <PageSection title="Character Details">
      <BlockGroup
        options={{
