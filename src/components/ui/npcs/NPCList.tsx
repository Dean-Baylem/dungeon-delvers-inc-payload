...
export default function NPCList({ list, isLoading }: Props) {
  return (
    <AnimatePresence mode="wait">
      <motion.ul
        key={list.map((i) => i.slug).join(',')}
        initial="hidden"
        animate="show"
        variants={{
      ...
      >
        {list.map((npc, index) => (
          ...
          >
            <Link
              href={`/npcs/${npc.slug}`}
              className={`block relative z-1 pt-60 h-full border-heading border-2 shadow-[2px_2px_4px_2px_rgba(0,0,0,0.45)] transition-transform duration-150 hover:scale-105 ${isLoading ? 'opacity-50 pointer-events-none' : ''}`}
            >
